...
namespace Unity.FPS.Gameplay
{
    ...
    public class PlayerWeaponsManager : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            int startingWeaponCount = Mathf.Min(1, StartingWeapons.Count);
        ...
        void Update()
        {
            ...
            {
                if (!activeWeapon.IsMeleeWeapon && !activeWeapon.AutomaticReload && m_InputHandler.GetReloadButtonDown() &&
                    activeWeapon.GetCurrentAmmo() < activeWeapon.ClipSize &&
                    activeWeapon.GetCarriedPhysicalBullets() > 0)
                {
                    IsAiming = false;
                ...
                IsAiming = !activeWeapon.IsMeleeWeapon && m_InputHandler.GetAimInputHeld();

                // handle shooting
                bool hasFired;
                if (activeWeapon.IsMeleeWeapon)
                {
                    ChainsawWeapon chainsaw = activeWeapon.GetComponent<ChainsawWeapon>();
                    if (chainsaw != null)
                    {
                        IContinuousMeleeAttackHandler continuous = GetComponent<IContinuousMeleeAttackHandler>();
                        hasFired = chainsaw.HandleInput(m_InputHandler.GetFireInputHeld(), continuous);
                    }
                    else
                    {
                        IMeleeAttackHandler melee = GetComponent<IMeleeAttackHandler>();
                        hasFired = m_InputHandler.GetFireInputDown() && melee != null &&
                                   melee.TryMeleeAttack(activeWeapon.MeleeDamage);
                    }
                }
                else
                {
        ...
        void LateUpdate()
        {
            ...
            // Some weapon view-model components create or reactivate children during Awake.
            // Enforce the inventory visibility invariant after all presentation updates.
            HideInactiveWeapons();
        }
        ...
        public bool AddWeapon(WeaponController weaponPrefab, bool equipImmediately = false)
        {
            ...
            for (int i = 0; i < m_WeaponSlots.Length; i++)
            {
                ...
                if (m_WeaponSlots[i] == null)
                {
                    ...
                    // Held weapon geometry is presentation-only. Mesh colliders on
                    // the SMG/MP5 and some melee models sit inside the player's own
                    // movement capsule and can make its forward capsule cast project
                    // away most of the requested velocity.
                    foreach (Collider collider in weaponInstance.GetComponentsInChildren<Collider>(true))
                    ...
                    // Hide other view models before starting the normal equip transition.
                    HideInactiveWeapons();
                    if (equipImmediately)
        ...
        public WeaponController GetFirstWeaponWithPhysicalBullets()
        {
            foreach (var weapon in m_WeaponSlots)
            {
                if (weapon != null && weapon.HasPhysicalBullets)
                {
                    return weapon;
                }
            }
        ...
        void HideInactiveWeapons()
        {
            for (int i = 0; i < m_WeaponSlots.Length; i++)
            {
                WeaponController weapon = m_WeaponSlots[i];
                if (weapon == null || i == ActiveWeaponIndex)
                    continue;
                ...
                bool rootVisible =
                    weapon.WeaponRoot != null &&
                    weapon.WeaponRoot.activeSelf;

                if (weapon.IsWeaponActive || rootVisible)
