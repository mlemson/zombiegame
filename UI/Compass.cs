...
namespace Unity.FPS.UI
{
    public class Compass : MonoBehaviour
    {
        ...
        void Update()
        {
            if (m_PlayerTransform == null && !TryBindLocalPlayer())
                return;
        ...
        bool TryBindLocalPlayer()
        {
            foreach (PlayerCharacterController candidate in FindObjectsByType<PlayerCharacterController>())
            {
                // Remote network avatars have this component disabled by NetworkPlayerOwnership.
                if (!candidate.enabled)
                    continue;

                m_PlayerTransform = candidate.transform;
                return true;
            }
