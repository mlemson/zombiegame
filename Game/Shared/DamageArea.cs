    ...
    public class DamageArea : MonoBehaviour
    {
            ...
            QueryTriggerInteraction interaction, GameObject owner)
        {
            ...
            foreach (Damageable uniqueDamageable in uniqueDamagedHealths.Values)
            {
                ...
                float healthBefore = uniqueDamageable.Health != null
                    ? uniqueDamageable.Health.CurrentHealth
                    : 0f;
                uniqueDamageable.InflictDamage(
                    damage * DamageRatioOverDistance.Evaluate(distance / AreaOfEffectDistance), true, owner);

                if (uniqueDamageable.Health != null && uniqueDamageable.Health.CurrentHealth < healthBefore)
                {
                    DamageEvent evt = Events.DamageEvent;
                    evt.Sender = owner;
                    evt.DamageValue = healthBefore - uniqueDamageable.Health.CurrentHealth;
                    evt.IsHeadshot = false;
                    evt.IsLethal = uniqueDamageable.Health.CurrentHealth <= 0f;
